...
export default function PlayingCard({
    ...
    return (
        <div 
            ...
            className={`relative flex justify-center items-center m-[2px] lg:m-[4px] 
                ${flipped ? 'bg-gradient-to-b from-green-dark to-green border-4' 
                : 'bg-white'}
                ${!disabled && !flipped && 'cursor-pointer lg:hover:shadow' }
                rounded-xl h-[140px] w-[100px] lg:h-[275px] lg:w-[200px] `}
                onClick={onClick}>
            {held && (
            ...
            {flipped ? (
                <div className='text-md lg:text-4xl absolute bottom-5 left-2'>
                    eCASINO
                </div>
            ) : (
                <div>
                    <div className={`${color} text-2xl lg:text-3xl absolute
                    left-3 lg:left-5 top-3 lg:top-5`}>
                        {rank}
                    </div>
                    <div className={`${color} text-2xl lg:text-3xl absolute
                    right-3 lg:right-5 bottom-3 lg:bottom-5 -rotate-180`}>
                        {rank}
                    </div>
                    <div className={`${color} text-5xl lg:text-8xl`}>
                        {suitsObj[suit]}
                    </div>
                </div>
            )}
